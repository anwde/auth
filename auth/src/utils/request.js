...
window.requestlist = new Map();
const get = function (url, data) {
  return http_request({ method: "get", url: url, ...data });
};
const post = function (url, data) {
  return http_request({ method: "post", url: url, ...data });
};
const put = function (url, data) {
  return http_request({ method: "put", url: url, ...data });
};
const patch = function (url, data) {
  return http_request({ method: "patch", url: url, ...data });
};
...
const http_request = async function (config) {
  ...
  try {
    const hash = utils.md5(options.url);
    if (window.requestlist.has(hash)) {
      // return Promise.reject("In the request");
    }
    options.cancelToken =
      options.cancelToken ||
      new axios.CancelToken((cancel) => {
        !window.requestlist.has(hash)&&window.requestlist.set(hash, cancel); 
      });
    ...
    if (config.cache) {
      ...
      if (data) {
        josn =
          JSON.parse(utils.decrypt(data, window.cs, hash.substr(8, 16))) || {};
      }
    }
    if (!josn) {
    ...
    window.requestlist.has(hash) && window.requestlist.delete(hash);
    if (josn.code === 10005) {
      notify.error({
        message: "通知",
