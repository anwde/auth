...
class Area extends Basic_Customer {
  page = {};
  pages={};
  ...
  async __init_index(d = {}) {
    ...
    b.buttons = [{
      title: "添加",
      url: `/customer/applications/spm/area/add?applications_id=${applications_id}`,
    },];
    ...
    this.init_lists("customer/applications/spm/area/lists", d, b, { app, pages });
  }
  ...
  async __init_add_edit(action) {
    ...
    if (action === "edit" && this.state.id) {
      const res = await webapi.request.get(
        "customer/applications/spm/area/get", {
        data: {
          id: this.state.id
        },
      }
  ...
  handle_submit = async (data = {}) => {
    ...
    const res = await webapi.request.post(
      "customer/applications/spm/area/dopost", {
      data
    }
    );
    if (res.code === 10000) {
      webapi.message.success(res.message);
      this.props.history.replace(`/customer/applications/spm/area/index?page_id=${state.page_id}`
  ...
  **/
  ...
  handle_delete(id) {
    this.handle_do_delete("customer/applications/spm/area", id);
  }
  /**
  * 复制
  **/
  ...
  handle_copy(copy_id) { 
    webapi.delete({
      url:'customer/applications/spm/area/copy',
      data:{id:copy_id},
      content:'确认复制吗?',
      success: (data) => {
        if (data.code === 10000) {
  ...
  **/
  ...
  handle_copy_to_module(copy_id) {

    this.setState({ copy_id, to_id: 0, modal_copy_to_module: true });
  }
  /**
  * 复制到页面
  **/
  ...
  * 复制到页面
  **/
  handle_copy_to_module_submit = async () => {
    const state = this.state;
    ...
    const res = await webapi.request.post(
      "customer/applications/spm/area/copy_to_module", {
      data
    }
    );
    if (res.code === 10000) {
      webapi.message.success(res.message);
      this.handle_copy_to_module_cancel();
    } else {
      webapi.message.error(res.message);
    }
        ...
        render: (item) => {
          return (
            <Space>
              ...
              <Link
                to={`/customer/applications/spm/area/edit/${item.id}?page_id=${item.page_id}`}
              >
    ...
    return (
      <>
        ...
        <Modal
          title="复制到模块"
          centered={true}
          open={state.modal_copy_to_module}
          onOk={this.handle_copy_to_module_submit}
          confirmLoading={this.props.server.loading}
          onCancel={this.handle_copy_to_module_cancel}
        >
          <Form
            layout='vertical'
          >
            <Form.Item label="请输入模块id">
              <InputNumber
                style={{
                  width: '100%',
                }}
    ...
    return (
      <Menu>
        ...
        <Menu.Item key="pages_3">
          <Button type="primary" shape="round" onClick={() => this.handle_copy_to_module(data.id)}>
            复制到页面
          </Button>
        </Menu.Item>
      </Menu>
