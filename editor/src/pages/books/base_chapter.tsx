...
import Basic_Books from "./base/basic";  
...
export default class Base_Chapter<P = {}, S = {}, SS = any> extends Basic_Books { 
    ...
    handle_content = async (id) => {
      const res = await webapi.request.get("books/chapters/content", {data:{
        chapter_id: id,
    ...
    handle_material = async (type) => {
      const state = this.state;
      const material = state.material;
      if (!material || material === '') {
        return 0;
      }
      const data = { book_id: state.book_id, chapter_id: state.data.id, type, name: material };
      const res = await webapi.request.post("books/home/material", { data });
      if (res.code === 10000) {
        webapi.message.success(res.message);
      } else {
        webapi.message.error(res.message);
        ...
        <>
          ...
          <Modal
            ...
            closeIcon={
              ...
              >
               <i className="icon icon-listok"></i>
              </a>
            }
