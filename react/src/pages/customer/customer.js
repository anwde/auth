...
import { Form, Input, Table, Button, Dropdown, Menu } from "antd";
const BREADCRUMB = {
  title: "商户管理",
  lists: [
    { title: "主页", url: "/" },
...
class Customer extends Basic_Customer {
   ...
   */
  __init_index(d = {}) {
    this.init_lists("customer/home/lists", d);
  }
  async __init_add_edit(action) {
    let b = {};
    let data = {};
    if (action === "edit" && this.state.id) {
      const res = await webapi.request.get("customer/home/get", {
        data: {
          id: this.state.id,
        },
      });
  ...
  handle_submit = async (data = {}) => {
    ...
    const res = await webapi.request.post("customer/home/dopost", { data });
    if (res.code === 10000) {
        ...
        render: (d) => {
          return (
            <div>
              ...
              <Dropdown
                overlay={this._render_dropdown_menus_action(d)}
                type="primary"
                shape="round"
              >
                <a className="btn btn-outline-info btn-circle btn-lg btn-circle ml-2" href="#!">
  ...
  _render_dropdown_menus_action = (data) => {
    return (
      <Menu>
        <Menu.Item>
          <Link to={`/customer/applications?customer_id=${data.id}`}>
