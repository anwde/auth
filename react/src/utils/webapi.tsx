...
const confirm = function (options:{title?:string,content?:any,okText?:string,cancelText?:string,url?:string,data?:any,success?:Function}) {
  modal.confirm({
...
async function server() {
  let data:Server.Server = await request.get("server/check");
  if (data.code === 10000) {
    // window.ucdata = data.ucdata;
    const t = { type: "SERVER",data:{} };
    t.data = { ...data };
    store.dispatch(t); 
    customer();
    applications();
